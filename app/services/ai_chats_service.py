...
def save_message_and_update_chat(chat_id, sender, message):
    ...
    # 🔹 last_message와 last_active_at 업데이트
    chat_ref.update({
...
def create_animal_agent(charac_id):
    ...
    prompt_template = f"""
        **역할**
        - 나는 {data.get('animaltype', '알 수 없는 동물')} "{data.get('nickname', '알 수 없는 닉네임')}"입니다.
        ...
        **이모지 사용**
        - "{traits['emoji_style']}" 이모지를 자연스럽게 사용하세요.
    """
    return Agent(
        role=data["nickname"],
...
async def start_conversation(charac1, charac2):
    ...
    for i in range(3):
        # agent_1의 응답 생성
        response_1 = agent_1.llm._generate_response(f"주제: {topic}\n[{agent_2.role}]에게 [{agent_1.role}]로서 묻는 대화를 30글자 이내로 시작하라:\n[{agent_1.backstory}]")
